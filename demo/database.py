...
class InMemoryDatabase:
    """Simple in-memory store to back demo endpoints.

    This is NOT persistent. It only exists for the life of the process and is
    intended to satisfy the demo API routes in `app.py`.
    """

    def __init__(self) -> None:
        self._auth_id_to_record: Dict[int, PriorAuth] = {}
        self._seed()

    def _seed(self) -> None:
        self._auth_id_to_record = {
            1: PriorAuth(
                auth_id=1,
                status="pending",
                steps=[
                    Step(1, "complete", "Collected patient information"),
                    Step(2, "pending", "Submitted initial prior auth request"),
                    Step(3, "blocked", "Awaiting insurer response"),
                ],
            ),
            2: PriorAuth(
                auth_id=2,
                ...
                steps=[
                    Step(1, "complete", "Validated coverage"),
                    Step(2, "in_progress", "Gathering clinical notes"),
                ],
            ),
            3: PriorAuth(
                auth_id=3,
                status="approved",
                steps=[
                    Step(1, "complete", "Submitted documentation"),
                    Step(2, "complete", "Insurer approved"),
                ],
            ),
        }

    # Query methods
    def get_prior_auths_by_status(self, status: str) -> List[Dict[str, Any]]:
        records = list(self._auth_id_to_record.values())
        if status and status.lower() != "all":
            records = [r for r in records if r.status.lower() == status.lower()]
        return [self._serialize(r) for r in records]

    def get_stats(self) -> Dict[str, Any]:
        total = len(self._auth_id_to_record)
        by_status: Dict[str, int] = {}
        for record in self._auth_id_to_record.values():
            by_status[record.status] = by_status.get(record.status, 0) + 1
        running_automations = sum(1 for r in self._auth_id_to_record.values() if r.automation_running)
        return {
            "total": total,
            "by_status": by_status,
            "automation_running": running_automations,
        }

    # Mutation methods
    def start_automation(self, auth_id: int) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record:
            return False
        record.automation_running = True
        if record.status == "pending":
            record.status = "in_progress"
        return True

    def pause_automation(self, auth_id: int) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record:
            return False
        record.automation_running = False
        return True

    def approve_step(self, auth_id: int) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record or not record.steps:
            return False
        # Approve the first non-complete step
        for step in record.steps:
            if step.step_status != "complete":
                step.step_status = "complete"
                break
        # If all steps complete, promote status to approved
        if all(s.step_status == "complete" for s in record.steps):
            record.status = "approved"
            record.automation_running = False
        return True

    def request_changes(self, auth_id: int, changes: str) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record or not changes:
            return False
        record.change_requests.append(changes)
        # Move status to "changes_requested" if not already terminal
        if record.status not in {"approved", "rejected"}:
            record.status = "changes_requested"
        return True

    def provide_feedback(self, auth_id: int, feedback: str) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record or not feedback:
            return False
        record.feedback.append(feedback)
        return True

    def update_step(self, auth_id: int, step_number: int, step_status: str, step_details: str) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record:
            return False
        existing: Optional[Step] = next((s for s in record.steps if s.step_number == step_number), None)
        if existing:
            existing.step_status = step_status
            existing.step_details = step_details
        else:
            record.steps.append(Step(step_number=step_number, step_status=step_status, step_details=step_details))
        # Simple heuristic: if any step is in progress/pending/blocked, set status in_progress
        if any(s.step_status in {"pending", "in_progress", "blocked"} for s in record.steps):
            record.status = "in_progress"
        elif all(s.step_status == "complete" for s in record.steps):
            record.status = "approved"
        return True

    def update_status(self, auth_id: int, new_status: str) -> bool:
        record = self._auth_id_to_record.get(auth_id)
        if not record or not new_status:
            return False
    ...
    # Helpers
    def _serialize(self, record: PriorAuth) -> Dict[str, Any]:
        return {
            "auth_id": record.auth_id,
            "status": record.status,
            "automation_running": record.automation_running,
            "steps": [
                {
                ...
                for s in sorted(record.steps, key=lambda x: x.step_number)
            ],
            "change_requests": list(record.change_requests),
            "feedback": list(record.feedback),
        }


# Export a singleton used by `app.py`
db = InMemoryDatabase()
